          ...
          (account) => `
            <tr class="border-b border-[#eee] dark:border-strokedark">
              <td class="px-4 py-5 xl:pl-11">
                <img
                  src="${account.userProfile.avatar}"
                  alt="${account.userProfile?.name || 'Avatar'}"
                  class="w-10 h-10 rounded-full"
                />
              </td>
              <td class="px-4 py-5">
                <a href="/users/${account.id}" class="text-sm font-medium text-black dark:text-white">
...
function handleBan(accountId) {
  if (confirm('Are you sure you want to ban this account?')) {
      fetch(`/accounts/user/ban/${accountId}`, {
          method: 'PATCH',
...
function handleUnban(accountId) {
  if (confirm('Are you sure you want to unban this account?')) {
      fetch(`/accounts/user/unban/${accountId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
      })
          .then((response) => {
          ...
          .catch((error) => {
              ...
              alert('Failed to unban the account.');
          });
  }
}
